...
use crate::data_lib::{Destination, Position, Velocity};
...
pub fn displacement(world:&World, owner:usize, target:usize, speed:f32, distance:f32) {
  ...
  let target_des = world.get_component_mut::<Destination>(target).unwrap();
  let target_vel = world.get_component_mut::<Velocity>(target).unwrap();
