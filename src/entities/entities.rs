...
//I think a better approach would be to have a vec of free spots/indexes and that we keep adding to whenever we delete an entity, 
//then when we add an entity, we look at the last available index in the free spots vec 
//and we insert the components at that index, then we simply just .pop() on the vec of free indexes, this should be faster for large amounts of entities
//This will be used a lot so making it as fast as possible is good
...
impl Entities{
  ...
  pub fn with_component(&mut self, data:impl Any) -> Result<&mut Self>  {
    ...
    if let Some(components) = self.components.get_mut(&typeid){
      ...
      *component = Some(Rc::new(RefCell::new(data)));
      ...
      return Err(CustomErrors::CreateComponentNeverCalled.into());
    };
    Ok(self)
  }
  ...
  //might wanna modify the add component to also be used for updating *or* make an update query based on it
