use crate::{
  game_data::{Armor, DamageType, Dead, Health, IncomingDamage, MagicResist},
...
///Subsystem which handles processing all damage entities recieve during a tick
/// and killing them if necessary. If the an was killed, give it a [`Dead`]
/// component and create an `EntityKilled` [`GameEvent`].
pub fn process_damage(world:&mut World) {
  ...
  for entity in entities {
    let incoming_damage = entity.get_component_mut::<IncomingDamage>().unwrap();
...
///Function which applies damage to an entity and checks if the damage killed
/// it. If the entity was killed, give it a [`Dead`] component and create an
/// `EntityKilled` [`GameEvent`].
pub fn apply_damage(world:&mut World, owner:usize, target:usize, damage:i32) {
  //Deal damage
  let is_zero;
  {
    let target_health = world.get_component_mut::<Health>(target).unwrap();
