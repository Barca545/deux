use winapi::um::{sysinfoapi::GetSystemTimePreciseAsFileTime,profileapi::{QueryPerformanceCounter,QueryPerformanceFrequency}};
use std::mem::zeroed;
type Minutes = i32;
type Seconds = f64;
type PerSecond = f64;
...
*/
  ...
  fn get_counts_per_second()->PerSecond{ //only reliable on a single core
    let freq = unsafe {
      let mut freq = zeroed();
...
#[cfg(test)]
  mod tests{
    use std::{mem::zeroed, time::Duration,thread::{sleep, self}};
