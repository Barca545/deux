...
use std::{env::var,fs::{self,DirBuilder},path::{Path,PathBuf}};
use walkdir::WalkDir;

fn main(){
  let out_dir = PathBuf::from(var("OUT_DIR").unwrap());
  // let manifest_dir = PathBuf::from(var("CARGO_MANIFEST_DIR").unwrap());
  let manifest_dir = PathBuf::from("C:\\Users\\Jamari\\Documents\\Hobbies\\Coding\\deux");
  
  let executable_path = locate_target_dir_from_output_dir(&out_dir)
    .expect("failed to find output dir")
