...
// -Is it better to set the time_since_last measurements to 0.0 after
...
impl ServerTime {
  ...
  fn get_display_game_duration(&self,) -> (u64, u64,) {
    let seconds_since_start = self.current_count - self.start_count;
    (
      seconds_since_start.as_secs() / 60,
      seconds_since_start.as_secs() % 60,
    )
  }
  ...
  loop{
    ...
    if server_time.should_update(){
      current_time = server_time.get_seconds_since_start();
      number_of_ticks+=1;
...
impl ServerTime {
  ...
  ///Decrements all of the game's [`BasicTimer`]s by the duration of one logic
  /// tick.
  fn decrement_real_remaining(&self,) {
    for timer in &self.timers {
      let mut borrowed_timer = timer.borrow_mut();
      borrowed_timer.decrement_real_remaining(self.tick_frequency.as_secs_f64(),);
  ...
  ///Decrements all of the game's [`BasicTimer`]s by the duration of one render
  /// tick.
  fn decrement_display_remaining(&self,) {
    for timer in &self.timers {
      let mut borrowed_timer = timer.borrow_mut();
      borrowed_timer.decrement_display_remaining(self.tick_frequency.as_secs_f64(),);
...
mod tests {
  ...
  fn time_does_ellapse() {
    ...
    while total_seconds <= 10.0 {
      ...
      total_seconds += time_elapsed.as_secs_f64();
