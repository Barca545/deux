use crate::{
  ecs::entities::{Component, Entities},
  errors::EcsErrors
};
use eyre::Result;
...
mod test {
  ...
  fn run_query() -> Result<()> {
    ...
    //not sure why this is returning an array but I kinda understand what is
    // happening the first element in the indexes array should be the map of the
    // first type
  ...
  fn query_for_entity_mutable() -> Result<()> {
    ...
    let entities:Vec<QueryEntity> = query.with_component::<Health>()?.run_entity();
