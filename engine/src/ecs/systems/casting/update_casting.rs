use crate::{
  component_lib::{CastQueue, Casting, PlayerState},
  ecs::{CommandBuffer, World},
  time::ServerTime,
};

// Refactor:
// -Once I migrate to BECS, I will need to use the command buffer to add the casting component
// -If there is a bottleneck experiment with deleting casting in this subsystem, try it in the cast abilities subsystem

///Query all all entities with [`CastQueue`]s.
/// Check if the entity has a [`Casting`] component.
/// If the ability in the `Casting` component is finished channeling, move the next [`BufferedAbilityCast`] out of the `CastQueue` and into the `Casting` component.
pub fn update_casting(world: &mut World) {
  ...
  for entity in entities {
    ...
    if casting.is_done() {
      //Buffer the command to delete the Casting component to the command buffer
      cmds.remove_component::<Casting>(entity.id);
    }
  }
  ...
  let mut query = world.query();
  let entities = query.with_component::<CastQueue>().unwrap().without_component::<Casting>().unwrap().run();

  for entity in entities {
    //Try to get the next buffered ability and create a new casting component from it
