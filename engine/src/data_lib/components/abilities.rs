...
// Refactor:
// -The CastQueue should track how long something is buffered and delete older
// buffers -Add the can cast checks into the create_ability_cast method. They
// should be utility functions.

///Structure holding information needed to create an [`AbilityCast`] -- the
/// information needed to cast an ability.
#[derive(Debug)]
...
impl AbilityMap {
  ...
  ///Creates an [`BufferedAbilityCast`] from the [`AbilityInfo`] in the
...
///Component that holds the [`AbilityCast`] currently being channeled by a
/// player and its `channel` [`Cooldown`].
#[derive(Debug, Clone)]
pub struct Casting {
