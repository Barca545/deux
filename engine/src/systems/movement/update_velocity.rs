...
use crate::data_lib::{Destination, Position, UnitSpeed, Velocity};

//Refactor
// -Can just query for velocity?

///Updates the [`Velocity`] component of all entities in the [`World`].
/// Sets calculates the `Velocity` as the vector betwenn an entity's
...
pub fn update_velocity(world:&World) {
  ...
  for entity in entities {
    ...
    let velocity = entity.get_component_mut::<Velocity>().unwrap();
