...
pub fn update_path(world:&mut World) {
  ...
  //   //If the entity's destination has been reached check for the next node in
  // the   // path
  //   if position.0 == destination.0 {
  //     let mut path = entity.get_component_mut::<Path>().unwrap();
  //     //If there is another node in the path, set the destination equal to
  // the node     if let Some(node) = path.next() {
  //       *destination = node;
  //     }
