...
// -Sending an attack command should be a different stage than setting the
// target? -Need to switch the player state at some point so the pending stuff
// is actually a cast time and not a buffered ability.
...
pub fn update_target(world:&World, entity:usize, mouse:MouseRay) {
  ...
  *target = Target(None);
  ...
  let queue = world.get_resource_mut::<GameEventQueue>();
