use crate::{data_lib::Target, math::MouseRay, scripting::LuaEntity};
...
pub fn run_scripts(world:&mut World, owner:&usize, ability_id:&usize, script:&String) {
  ...
  //   let lua = world.get_resource::<Rc<Lua>>().clone();
  //   lua
  //     .scope(|scope| {
  ...
  //       lua.globals().set("ability",
  // scope.create_userdata_ref(&ability_id)?)?;

  //       //Add the world
  //       // lua.globals().set("world",
  // scope.create_userdata_ref_mut(world)?)?;
...
pub fn eval_scripts_mouse<'lua, T:for<'scope> FromLua<'scope>>(world:&mut World, owner:&usize, target:&Target, mouse:&MouseRay, script:&String) -> Option<T> {
  todo!()
  // let owner_id = LuaEntity::from(owner);
