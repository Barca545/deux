use mlua::UserData;
use crate::{component_lib::{Destination, Path, Position}, ecs::World};

//Need to add the destinations in reverse order because they will be fetched by poping the vector
// one way to do this is to set the entity's current position as the goal and the destination as the start point
