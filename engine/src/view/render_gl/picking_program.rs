use gl::{Gl, types::{GLint, GLuint}};

use crate::{ecs::World, math::{Transforms, math::Vec3}};
...
impl PickingProgram{
  ...
  pub fn draw_start_cb(&self,draw_index:GLint){
    //this could maybe be a function on the main program struct
    unsafe{
      self.gl.Uniform1ui(self.draw_index_location, draw_index as GLuint)
    }
  }
  
  pub fn set_object_index(&self,object_index:GLint){
    unsafe{
      self.gl.Uniform1ui(self.object_index_location, object_index as GLuint)
    }
  }
}