...
use wgpu::{
  ...
  DepthStencilState, Device, DeviceDescriptor, Face, Features, FragmentState, FrontFace, Instance,
  InstanceDescriptor, LoadOp, MultisampleState, Operations, PipelineLayout,
  ...
  ShaderModule, ShaderStages, StencilState, StoreOp, Surface, SurfaceConfiguration,
...
impl Renderer {
  pub async fn new(canvas:Arc<Canvas<sdl2Window,>,>,) -> Self {
    ...
    let surface = unsafe {
      instance
        .create_surface_unsafe(SurfaceTargetUnsafe::from_window(canvas.window(),).unwrap(),)
        .unwrap()
    };
    ...
    let pipeline_layout = device.create_pipeline_layout(&PipelineLayoutDescriptor {
      ...
      bind_group_layouts:&[
        &Self::texture_bind_group_layout(&device,),
  ...
  pub fn update(&mut self, world:&World,) {
    ...
    for entity in entities {
      ...
      let render_position = calculate_render_position(
        *previous_position,
        *position,
        server_time.get_interpolation_factor(),
      );
  ...
  pub fn render(&self,) -> Result<(),> {
    ...
    self.queue.write_buffer(
      &self.camera_buffer,
      0,
      bytemuck::cast_slice(&self.frame.pv_mat,),
    );
