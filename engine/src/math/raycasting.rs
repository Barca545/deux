use glm::{self,vec3, inverse, vec4};
...
impl RayCast{
  pub fn new(origin:Vec3,end:Vec3)->RayCast{
    ...
    RayCast {origin, direction, inverse_direction}
  }
  
  ///Calculates the point of intersection between a ray cast in world coordinates and the ground.
  /// 
  ...
  pub fn calculate_ray_plane_intersection(&self,plane_normal:Vec3,plane_origin:Vec3)->Vec3{
  ...
  pub fn calculate_ray_ground_intersection(&self)->Vec3{
    let plane_normal:Vec3 = vec3(0.0,1.0,0.0); //I think the normal to plane xz is this
    let plane_origin:Vec3 = vec3(0.0,0.0,0.0); //this is "plane_point" in the tutorial. I think it can just be any point on the plane?
...
mod test {
  use glm::{Vec3, vec3};
  use crate::{ecs::world_resources::ScreenDimensions, view::camera::Camera, math::Transforms};
  use super::MouseRay;
  #[test]
  fn ray_plane_intersection(){
    ...
    let plane_origin:Vec3 = vec3(0.0, 0.0, 5.0); //this is "plane_point" in the tutorial. I think it can just be any point on the plane?
  ...
  fn mouse_intersection(){
    ...
    //this seems to be wrong, should return 0,0. Do this math by hand and dbg each line of the code to isolate the divergence-0-/9[p09-p[.9578-9-p[.]]]
    //issue seems to have been I was accidently scaling by distance twice test some more points to confirm``
    let intersection:Vec3 = mouse_ray.calculate_ray_ground_intersection();
    dbg!(intersection);
    //set the direction to straight down, the point of intersection should just be the input point with y = 0
    //if this is not the result the math error is happening elsewhere
  }
}